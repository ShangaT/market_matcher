...
class MagnitParser:
    ...
    def __init__(self, stock: Stock, device_id='d86dpcdvok') -> None:
        self.stock = stock
        client = Session()
        client.headers['User-Agent'] = 'Mozilla/5.0 (Linux; U; Linux i674 ) Gecko/20130401 Firefox/62.8'
        client.headers['X-Device-Id'] = device_id
    ...
    def total_products(self):
        ...
        r = self.client.post(url, json={'categoryIDs': [], 'includeForAdults': True,
                            #  "shopType": "1", 
                             "storeCodes": [self.stock.stock_id], "filters": []})
        
        return r.json()['pagination']['totalCount']
    
    def fetch_categories(self):
        url = f'https://web-gateway.middle-api.magnit.ru/v2/goods/categories?StoreCode={self.stock.stock_id}'
        r = self.client.get(url)

        data = r.json()
        return [ctg for ctg in data if ctg['code']]
    ...
    def fetch_products(self, category_ids: list, v=False, sleep_sec = 1) -> list:
        ...
        while page <= total_pages:
            ...
            r = self.client.post(url, json={"categoryIDs": category_ids, "includeForAdults": True, "onlyDiscount": False, "order": "desc", "pagination": {
                                "number": page, "size": page_size}, "shopType": "1", "sortBy": "price", "storeCodes": [str(self.stock.stock_id)], "filters": []})
            ...
            for p in data['goods']:
                if 'offers' in p and len(p['offers']) > 0:
    ...
    def start(self) -> list[Product]:
        ...
        for ctg in ctgs:
            time.sleep(1)
            print(MagnitParser.store_code, self.stock.region, ctg['name'])
            ctg_ids = self.get_subcategory_ids(ctg)
            # print(ctg_ids)
            products = self.fetch_products(
                category_ids=ctg_ids, v=False, sleep_sec=0.5)
